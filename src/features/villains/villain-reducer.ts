...
export const villainReducer = (state: IVillainState = initialState, action: IAction): IVillainState => {
  switch (action.type) {
    ...
    case VillainActionTypes.UPDATE_VILLAIN_SUCCESS:
      return {
        ...state,
        isLoading: false,
        villains: state.villains.map(villain => (villain.id === action.payload.id ? action.payload : villain)),
      }
