...
function* updatingHero({ payload: updatedHero }: any) {
  try {
    ...
    yield put({ type: HeroActionTypes.UPDATE_HERO_SUCCESS, payload: updatedHero })
  } catch (e) {
    yield put({
...
export function* heroSaga() {
  yield all([watchFetchingHeroes(), watchRemovingHero(), watchAddingHero(), watchUpdatingHero()])
}
